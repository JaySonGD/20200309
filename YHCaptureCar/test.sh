...
#工程名 将XXX替换成自己的工程名
project_name=YHCaptureCar

#scheme名 将XXX替换成自己的sheme名
scheme_name=YHCaptureCar
...
git log  --date=format:'%Y-%m-%d %H:%M:%S'  --pretty=format:'%h: %s  (%ad )  <%an>' --abbrev-commit >> ${exportIpaPath}/changelog
exit 0
#
#OLD_IFS="$IFS" #保存旧的分隔符
#IFS="\r"
...
#for i in "${!array[@]}"; do
#j=$(($i+1))
#echo "$j. ${array[i]}" >> ${exportIpaPath}/changelog
##echo -e "\n" >> ${exportIpaPath}/changelog
#done
#exit 0
...
open -a "/Applications/Safari.app"  http://fir.im/captureCar

#$GIT_LOG
#curl -i "http://127.0.0.1/app/public/api/appSumbit2?debug=99&json" -X POST -d "log=$GIT_LOG"
