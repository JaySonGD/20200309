...
- (void)initMapView
{
    ...
    ///初始化地图
    MAMapView *mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, NavbarHeight, screenWidth, screenHeight-NavbarHeight)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    selectmodel.isOnline = 1;
    [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake([selectmodel.latitude doubleValue], [selectmodel.longitude doubleValue]) animated:YES];
...
-(void)initData
{
    [YHHelpSellService stationListOnComplete:^(NSMutableArray<YHReservationModel *> *models) {
        self.models = models;
...
-(void)mapView:(MAMapView *)mapView didSelectAnnotationView:(MAAnnotationView *)view
{
    ...
    for (YHReservationModel *reservationM in self.models) {
        ...
        if ([selectView isKindOfClass:[YHAnnotationView class]] && [selectView.ID isEqualToString:reservationM.ID]) {
            ...
            //获得大头针的那个模型
            NSLog(@"点击----%@----%@----%@",reservationM.name,reservationM.tel,reservationM.addr);
            reservationM.isOnline = 1;
