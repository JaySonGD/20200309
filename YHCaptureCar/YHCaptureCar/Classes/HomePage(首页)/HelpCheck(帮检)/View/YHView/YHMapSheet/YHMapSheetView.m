...
+ (instancetype)showAnnotation:(YHReservationModel *)model{
    ...
    view.contactNameLB.text = model.contactName;
    view.telLB.text = model.tel;
    view.addrLB.text = model.addr;
    
    if (!model.isOnline) {
        view.bookBtn.enabled = NO;
        view.bookBtn.backgroundColor = [UIColor colorWithRed:0.79 green:0.79 blue:0.80 alpha:1.00];
    }
