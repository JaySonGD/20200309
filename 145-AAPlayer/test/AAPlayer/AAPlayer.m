...
@property (nonatomic, strong) AAFullController *fullVC;
...
- (void)setUI{
    ...
    _timeObserverToken = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:dispatch_get_main_queue() usingBlock:
                          ^(CMTime time) {
                              ...
                              if(!self.playPauseButton.isEnabled) return;
        ...
        case UIGestureRecognizerStateChanged:{ // 正在移动
            switch (self.panDirection) {
                case AAMoveDirectionHorizontal:{
                    if(self.playPauseButton.isEnabled)
...
- (void)hiddenSatusBar:(BOOL)hidden{
    self.viewController.ddStatusBarHidden = hidden;
    return;
...
- (void)enterFullscreenLeft {
    ...
    if (self.state == AAPlayerStateLeft || self.state == AAPlayerStateAnimating) {
        return;
    }
    
    self.state = AAPlayerStateAnimating;
    
    if(CGRectEqualToRect(CGRectZero, self.playViewSmallFrame) || !self.playViewParentView) {
    ...
    if (!self.fullVC) {
        AAFullController *vc = [AAFullController new];
        vc.transitioningDelegate = self.customAnimator;
...
- (void)enterFullscreenRight {
    ...
    if (self.state == AAPlayerStateRight || self.state == AAPlayerStateAnimating) {
        return;
    }
    self.state = AAPlayerStateAnimating;
...
- (void)exitFullscreen {
    ...
    NSLog(@"%s--竖屏--exitFullscreen", __func__);
    
    if (self.state == AAPlayerStateSmall || self.state == AAPlayerStateAnimating) {
        return;
    }
...
//        [self.playViewParentView addSubview:self];
        ...
        self.fullVC = nil;
        NSLog(@"%s--竖屏--dismissViewControllerAnimated--%@", __func__,NSStringFromCGRect(self.frame));
...
- (void)setAdView:(UIView *)adView{
    ...
    UIButton *cutDownButton = [UIButton buttonWithType:UIButtonTypeCustom];
    cutDownButton.frame = CGRectMake(self.bounds.size.width - 75, 15, 55, 20);
    
    cutDownButton.layer.cornerRadius = 10;
    
    cutDownButton.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.35];
    //cutDownButton.titleLabel.font = [UIFont systemFontOfSize:14];
    cutDownButton.titleLabel.font = [UIFont systemFontOfSize:10];
    
    [cutDownButton addTarget:self action:@selector(close:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:cutDownButton];
    
    cutDownButton.autoresizingMask =
    UIViewAutoresizingFlexibleLeftMargin   |
//    UIViewAutoresizingFlexibleWidth        |
//    UIViewAutoresizingFlexibleRightMargin  |
//    UIViewAutoresizingFlexibleTopMargin    |
//    UIViewAutoresizingFlexibleHeight       |
    ...
    [self openCountdown:cutDownButton];
...
//FIXME:  -  开启倒计时效果
-(void)openCountdown:(UIButton *)cutDownButton{
    
    
    __block NSInteger time = 8; //倒计时时间
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        if(time <= 0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //设置按钮的样式
                [self close:cutDownButton];
            });
            
        }else{
            
            NSInteger seconds = time % (8 + 1);
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置按钮显示读秒效果
                [cutDownButton setTitle:[NSString stringWithFormat:@"跳过 %ld",(long)seconds] forState:UIControlStateNormal];
            });
            time--;
...
- (void)changeRotate:(NSNotification*)noti {
    ...
    if(selfVC && topVC && topVC != selfVC) return;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"asset"]) {
        ...
        self.fullButton.enabled = YES;//hasValidDuration;
        ...
        UIImage *buttonImage = (newRate == 1.0) ? [UIImage imageNamed:@"fullplayer_icon_pause"] : [UIImage imageNamed:@"fullplayer_icon_play"];
...
- (BOOL)prefersStatusBarHidden{
    NSLog(@"%s----UITabBarController", __func__);
...
- (BOOL)prefersStatusBarHidden{
    NSLog(@"%s----UINavigationController", __func__);
...
- (BOOL)prefersStatusBarHidden{
    NSLog(@"%s----UIViewController-%d", __func__,self.ddStatusBarHidden);
