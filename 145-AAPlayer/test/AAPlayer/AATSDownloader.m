...
#if __has_include("AARequest.h")
#import "AARequest.h"
#endif

#import <CommonCrypto/CommonCrypto.h>
#if __has_include(<HTTPServer.h>)
#import <HTTPServer.h>
#endif

@interface AATSDownloader ()
...
@implementation AATSDownloader

+ (AATSDownloader *)sharedManager {
    static dispatch_once_t once;
    static id instance;
    dispatch_once(&once, ^{
        AATSDownloader *obj = [self new];
        obj.maxCount = 20;
        instance = obj;
    });
    return instance;
...
#if __has_include(<HTTPServer.h>)
    NSString *file = [NSString stringWithFormat:@"%@.m3u8",[[AATSDownloader sharedManager] md5:url]];
    NSString *filePath = [[[AATSDownloader sharedManager] mainDirectory] stringByAppendingPathComponent:file];
    if( [[NSFileManager defaultManager] fileExistsAtPath:filePath] ){
        if(![[AATSDownloader sharedManager] openServer]) return url;
        return [NSString stringWithFormat:@"http://127.0.0.1:%hu/%@",[AATSDownloader sharedManager].httpServer.listeningPort,file];
    }
#endif
...
+ (NSMutableArray *)videoList{
    ...
    [urls enumerateObjectsUsingBlock:^(NSString * url, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSString *filePath = [NSString stringWithFormat:@"http://127.0.0.1:%hu/%@",[AATSDownloader sharedManager].httpServer.listeningPort,file];//[[[FFTSDownloader sharedManager] mainDirectory] stringByAppendingPathComponent:file];
    ...
    NSArray <NSString *>*turls = [[[AATSDownloader sharedManager] tasks] valueForKeyPath:@"url"];
    ...
    [list addObjectsFromArray:[AATSDownloader sharedManager].tasks];
...
//    [[AARequest request] getJson:@"getTsList" parameter:@{@"url":url} success:^(id respones) {
//        
...
//        
//        if(!ts || !total){
//            completionBlock([NSError errorWithDomain:NSURLErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey:@"下载数据为空"}]);
//            return ;
//        }
//        
//        [self dowloadWithTs:ts url:url total:total  fileName:fileName processBlock:processBlock completionBlock:completionBlock];
//        
//    } error:^(NSError *error) {
...
//
//        NSData *data = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://129.204.117.172/api/d?debug=99&url=%@",url]]];
//
//        if(!data){
...
- (void)dowloadWithTs:(NSArray<NSString *> *)ts
    ...
    if ([AATSDownloader isDownloaded:url]) {
...
- (void)downloadVideoWithArr:(NSArray *)listArr andIndex:(NSInteger)index{
    ...
    NSURLSessionDownloadTask *downloadTask = [self downloadURL:downloadURL
...
- (void)convert {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kDownloadProgressNotification object:nil userInfo:@{@"url":self.url,@"process":@(1),@"speed":[self speedString:0]}];
...
+ (void)delete:(NSString *)url{
    ...
    NSString *tsDir = [[[AATSDownloader sharedManager] mainDirectory] stringByAppendingPathComponent:[[AATSDownloader sharedManager] md5:url]];
    
    NSString *file = [NSString stringWithFormat:@"%@.m3u8",[[AATSDownloader sharedManager] md5:url]];
    NSString *filePath = [[[AATSDownloader sharedManager] mainDirectory] stringByAppendingPathComponent:file];
    
    NSString *tsFile = [NSString stringWithFormat:@"%@.ts",[[AATSDownloader sharedManager] md5:url]];
    NSString *tsFilePath = [[[AATSDownloader sharedManager] mainDirectory] stringByAppendingPathComponent:tsFile];
...
- (BOOL)openServer {
    ...
    if (self.httpServer.isRunning) {
        NSLog(@"服务器已经启动 端口:%hu",[self.httpServer listeningPort]);
        return YES;
    }
