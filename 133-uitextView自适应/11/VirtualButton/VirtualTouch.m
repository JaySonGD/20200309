...
#define SCREEN_WIDTH    CGRectGetWidth([VirtualTouch screenBounds])
#define SCREEN_HEIGHT   CGRectGetHeight([VirtualTouch screenBounds])

@interface VirtualTouch ()
...
- (void)setTapBlock:(void (^)(VirtualTouch *))tapBlock {
    _tapBlock = tapBlock;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.isDragging) {
        ...
        CGFloat x = centerx - self.width*0.5;
        CGFloat y = centery - self.height*0.5;
        UIEdgeInsets inset = UIEdgeInsetsZero;
        if (@available(iOS 11.0, *)) {
            inset = self.window.safeAreaInsets;
        }
        if (y<inset.top) {
            y = inset.top;
        }
        if (y + self.height >= screenHeight) {
            y = screenHeight - self.height - inset.bottom;
        }
        if (x <= 0) {
            x = SPACE_SIDE;
        }
        if (x + self.width >= screenWidth) {
            x = screenWidth - self.width - SPACE_SIDE;
        }
        
        centerx = x +  self.width*0.5;
        centery = y + self.height*0.5;
...
+ (void)removeAllFromKeyWindow {
    ...
    for (UIWindow *window in [windows reverseObjectEnumerator])
    {
        for (id view in [window subviews]) {
            if ([view isKindOfClass:[VirtualTouch class]]) {
...
+ (CGRect)screenBounds
{
    ...
    if (UIInterfaceOrientationIsLandscape(interfaceOrientation) && sWidth < sHeight) {
        CGFloat tmp = sWidth;
        sWidth = sHeight;
        sHeight = tmp;
    }
    
    return CGRectMake(0, 0, sWidth, sHeight);
}
