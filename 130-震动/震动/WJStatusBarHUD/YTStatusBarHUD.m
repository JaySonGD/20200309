...
/** HUD控件默认会停留多长时间 */
static CGFloat const WJHUDStayDuration = 1.0;
...
+ (void)showImage:(UIImage *)image text:(NSString *)text{
    ...
    window_.backgroundColor = [UIColor colorWithRed:0.00 green:0.63 blue:0.35 alpha:1.00];//[UIColor blackColor];
...
+ (NSString *)cheakSuccessImageName:(NSString *)imageName{
    ...
    if (imageName.length < 1 || imageName == nil || imageName == NULL) {
        imageName = @"YTStatusBarHUD_success";
    }
    return imageName;
}
...
+ (NSString *)cheakErrorImageName:(NSString *)imageName{
    ...
    if (imageName.length < 1 || imageName == nil || imageName == NULL) {
        imageName = @"YTStatusBarHUD_error";
    }
    return imageName;
}
...
+ (NSString *)cheakWarningImageName:(NSString *)imageName{
    ...
    if (imageName.length < 1 || imageName == nil || imageName == NULL) {
        imageName = @"YTStatusBarHUD_warning";
    }
    return imageName;
}
