...
/** 滑块的大小 */
static const CGFloat kSliderBtnWH = 16.0;
/** 间距 */
static const CGFloat kProgressMargin = 2.0;
/** 进度的高度 */
static const CGFloat kProgressH = 2.0;//1.0;
/** 拖动slider动画的时间*/
static const CGFloat kAnimate = 0.3;

@implementation DDSliderButton
...
/** 滑块 */
@property (nonatomic, strong) DDSliderButton *sliderBtn;
...
- (void)layoutSubviews {
    ...
    CGPoint center = self.sliderBtn.center;

    CGFloat centerX =  CGRectGetWidth(self.bgProgressView.frame) * self.value - min_w*0.5;
    centerX = (centerX > min_w*0.5)? centerX : min_w*0.5;
    center.x = centerX;

    self.sliderBtn.center = center;
    
    min_x = 0;
    min_y = 0;
    if (self.sliderBtn.hidden) {
        min_w =CGRectGetWidth(self.bgProgressView.frame) * self.value;
    } else {
        min_w = center.x + min_w*0.5;
    }
    min_h = self.sliderHeight;
    ...
    min_x = 0;
    min_y = 0;
    min_w = CGRectGetWidth(self.bgProgressView.frame) * self.bufferValue;
    min_h = self.sliderHeight;
    ...
    self.bgProgressView.center = CGPointMake(self.bgProgressView.center.x, min_view_h * 0.5);
    self.bufferProgressView.center = CGPointMake(self.bufferProgressView.center.x, min_view_h * 0.5);
    self.sliderBtn.center = CGPointMake(self.sliderBtn.center.x, min_view_h * 0.5);
    self.sliderProgressView.center = CGPointMake(self.sliderProgressView.center.x, min_view_h * 0.5);
...
- (void)setValue:(float)value {
    ...
    _value = value;
    if (self.sliderBtn.hidden) {
        CGRect frame = self.sliderProgressView.frame;
        frame.size.width = frame.size.width * value;
        self.sliderProgressView.frame = frame;
    } else {
        
        
        CGFloat centerX =  CGRectGetWidth(self.bgProgressView.frame) * value - self.thumbSize.width*0.5;
        centerX = (centerX > self.thumbSize.width*0.5)? centerX : self.thumbSize.width*0.5;
        ...
        CGRect frame = self.sliderProgressView.frame;
        frame.size.width = CGRectGetMaxX(self.sliderBtn.frame);
        self.sliderProgressView.frame = frame;
    }
}
...
- (void)setBufferValue:(float)bufferValue {
    ...
    frame.size.width = CGRectGetWidth(self.bgProgressView.frame) * bufferValue;
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    CGRect frame = self.bgProgressView.frame;
    frame.size.height = sliderHeight;
    self.bgProgressView.frame = frame;
    
    frame = self.bufferProgressView.frame;
    frame.size.height = sliderHeight;
    self.bufferProgressView.frame = frame;
    
    frame = self.sliderProgressView.frame;
    frame.size.height = sliderHeight;
    self.sliderProgressView.frame = frame;
...
- (void)startAnimating {
    ...
    scaleAnimation.toValue = @(CGRectGetWidth(self.frame) * 10);
...
- (void)sliderBtnDragMoving:(UIButton *)btn point:(CGPoint)touchPoint {
    ...
    CGFloat value = (point.x - CGRectGetWidth(btn.frame) * 0.5) / CGRectGetWidth(self.bgProgressView.frame);
    // value的值需在0-1之间
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    // 获取进度
    CGFloat value = (point.x - CGRectGetWidth(self.sliderBtn.frame) * 0.5) * 1.0 / CGRectGetWidth(self.bgProgressView.frame);
...
- (DDSliderButton *)sliderBtn {
    if (!_sliderBtn) {
        _sliderBtn = [DDSliderButton buttonWithType:UIButtonTypeCustom];
