...
//       NSForegroundColorAttributeName:[UIColor whiteColor]}];
    ...
    if(
       (
        (self.navigationItem.leftBarButtonItem != nil && !self.navigationItem.leftBarButtonItem.customView.hidden)
...
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion{
    if ([viewControllerToPresent  isKindOfClass:NSClassFromString(@"PCAlertViewController")]) {
        
        if (![[PCAlertViewManager manager].tasks containsObject:viewControllerToPresent]) {
            [[PCAlertViewManager manager].tasks addObject:viewControllerToPresent];
        }
        
//        if ([PCAlertViewManager manager].tasks.count != 1) {
//            return;
//        }
        if ([PCAlertViewManager manager].isPresent) {
                  return;
        }
        
        [[UIApplication sharedApplication].keyWindow.rootViewController addChildViewController:viewControllerToPresent];
        [[UIApplication sharedApplication].keyWindow.rootViewController.view addSubview:viewControllerToPresent.view];
        [PCAlertViewManager manager].isPresent = YES;
