...
{
    const char *webSite = [NETWORK_URL_V2 cStringUsingEncoding:NSASCIIStringEncoding];
    if (webSite == NULL) {
        return false;
    }
    // Get host entry info for given host
    return [self reachabilityWithHostName_v:V2] != NotReachable && (gethostbyname(webSite));
}
