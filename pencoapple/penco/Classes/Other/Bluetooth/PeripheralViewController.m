...
-(void)loadData{
    ...
    CBPeripheral *currPeripheral = [baby retrievePeripheralWithUUIDString:[YHTools buluetoothName]];
...
-(void)insertSectionToTableView:(CBService *)service{
    YHLog(@"搜索到服务:%@",service.UUID.UUIDString);
...
-(void)insertRowToTableView:(CBService *)service{
    ...
    if (sect != -1) {
        ...
        for (int row=0;row<service.characteristics.count;row++) {
            ...
            YHLog(@"add indexpath in row:%d, sect:%d",row,sect);
        }
        PeripheralInfo *curInfo =[self.services objectAtIndex:sect];
        YHLog(@"%@",curInfo.characteristics);
