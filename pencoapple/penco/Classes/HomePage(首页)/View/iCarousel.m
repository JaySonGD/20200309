...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeInvertedRotary:
        {
            ...
            CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:MAX(_itemWidth * spacing / 4.0, _itemWidth * spacing / 4.0 / tanf(arc/4.0/count))];
            ...
            if (_vertical)
            {
                CGFloat tz = radius * cos(angle) - radius;
...
//                    offset -= 1;
//                }else if (offset <-2){
                CGFloat x = (offset>0)? 1.0 : -1.0;
                if (offset == 0) {
                    x = 0;
                }
                     CGFloat y = offset*1 + (offset*0.1);
                YHLog(@"   %f   %f   %f", offset,tz,y);

//                }

                
                UIView *v = _itemViews[@(_previousItemIndex)];
                CGFloat h = v.bounds.size.height;
                
                 y = offset*1 + (offset*20.0/h);

                YHLog(@"%s---%f", __func__,h);
                return CATransform3DTranslate(transform, 0.0, y  * h, tz);

                return CATransform3DTranslate(transform, 0.0, y  * _itemWidth, tz);
