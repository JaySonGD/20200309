        ...
        case .vertical:
            ...
            targetRect.origin = CGPoint(x: rect.origin.x, y: rect.origin.y-rect.height/2)
                        ...
                        for attribute in attributes {
                            ...
                            let difference = collectionView.center.y - cellY + collectionView.frame.origin.y+collectionView.superview!.frame.origin.y
                            ...
                            var transform = CATransform3DIdentity
                            let distanceFromCenter = fabs(collectionView.frame.origin.y+collectionView.superview!.frame.origin.y+collectionView.center.y - cellY)

//                            print(attribute.indexPath.item);
//                            print(cellY)

                            
                            if cellY < (itemSize.height * 0.5 + collectionView.frame.origin.y + collectionView.superview!.frame.origin.y) {
                                let yDelta = getYDelta(y: cellY - collectionView.frame.origin.y - collectionView.superview!.frame.origin.y)
                                transform = CATransform3DTranslate(transform, 0.0, yDelta, 0.0)
//                                transform = CATransform3DScale(transform, scaleFactor, scaleFactor, 1.0)
                                //let scale = sqrtDistributor(x: distanceFromCenter, threshold: periodH*0.5, xOrigin: periodH*0.6) * 0.2 + 0.8
                            ...
                            if cellY > (collectionView.bounds.height - itemSize.height * 0.5 + collectionView.frame.origin.y+collectionView.superview!.frame.origin.y) {
                                let yDelta = -getYDelta(y: collectionView.bounds.height-cellY+collectionView.frame.origin.y+collectionView.superview!.frame.origin.y)
                                transform = CATransform3DTranslate(transform, 0.0, yDelta, 0.0);
//                                transform = CATransform3DScale(transform, scaleFactor, scaleFactor, 1.0)
