...
open class VegaScrollFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        dynamicItems?.forEach { item in
            ...
            transformItemIfNeeded1(y: convertedY, item: item)
...
//        }
        ...
        let minH = collectionView!.bounds.height - itemSize.height*0.5
        let maxH = collectionView!.bounds.height

        guard  itemSize.height > 0, minH < y else {
            return
        }
		
        let scaleFactor: CGFloat = scaleDistributor(x: maxH - y)

        let yDelta = -getYDelta(y: maxH - y)
...
//        item.alpha = alphaDistributor(x: maxH - y)
    ...
    private func transformItemIfNeeded1(y: CGFloat, item: UICollectionViewLayoutAttributes) {
                guard itemSize.height > 0, y < itemSize.height * 0.5 else {
                    return
                }
        ...
        item.transform3D = CATransform3DTranslate(transformIdentity, 0, yDelta, 0)//向下移动yDelta
        item.transform3D = CATransform3DScale(item.transform3D, scaleFactor, scaleFactor, scaleFactor)
...
//            springBehaviour.frequency = 1.0
    ...
    private func alphaDistributor(x: CGFloat) -> CGFloat {
		return distributor(x: x, threshold: itemSize.height * 0.5, xOrigin: -itemSize.height*10)
    }
