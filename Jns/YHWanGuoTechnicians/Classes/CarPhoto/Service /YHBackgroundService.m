...
//DEFINE_SINGLETON_FOR_CLASS(YHBackgroundService);
...
-(NSString*)uploadTaskRequestBody2:(NSData *)data
    ...
    for (int i=0; i<allKeys.count; i++)
    {
        ...
        NSString* object=[NSString stringWithFormat:@"%@",[dictionary objectForKey:allKeys[i]]];
...
//    if([[NSFileManager defaultManager] createFileAtPath:tempFilePath contents:data attributes:nil]){
//        NSLog(@"%s---保存上传临时文件成功", __func__);
//    }
    NSLog(@"%s --原文件:-%lu", __func__,data.length/1024);
    NSLog(@"%s --临时文件:-%lu", __func__,totlData.length/1024);
...
didCompleteWithError:(nullable NSError *)error{
    ...
    if (error != nil){
        ...
        id obj = self.responseData? [NSJSONSerialization JSONObjectWithData:self.responseData
                                                 options:NSJSONReadingMutableContainers
                                                                      error:NULL] : nil;
        [[NSFileManager defaultManager] removeItemAtPath:self.tempPath error:NULL];
        NSLog(@"%s--删除上传临时文件---%@", __func__,self.tempPath);
