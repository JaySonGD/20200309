...
-(void)getPrepayId{
    ...
    __block NSDictionary *loginInfo = [(AppDelegate*)[[UIApplication sharedApplication] delegate] loginInfo];
    if (loginInfo) {
        [SVProgressHUD showWithStatus:@"加载中..."];
        [[YHNetworkPHPManager sharedYHNetworkPHPManager] prepay:loginInfo[@"userId"] type:_type byVin:_vin onComplete:^(NSDictionary *info)  {
            NSString *retCode = info[@"retCode"];
            [SVProgressHUD dismiss];
            if ([retCode isEqualToString:@"0"]) {
                NSDictionary *result = info[@"result"];
                NSString *prepay_id = result[@"prepayId"];
                [self payByPrepayId:prepay_id];
            }else{
