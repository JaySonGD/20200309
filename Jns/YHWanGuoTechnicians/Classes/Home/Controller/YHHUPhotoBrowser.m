...
#import "YHHUPhotoBrowserCell.h"
#import "UIImageView+HUWebImage.h"
#import <HUPhotoBrowser/HUWebImageDownloader.h>

@interface YHHUPhotoBrowser () <UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout> {
...
@property (nonatomic)BOOL isColseBtn;
@end

@implementation YHHUPhotoBrowser

- (void)dealloc {
    self.collectionView.delegate = nil;
    [[NSNotificationCenter defaultCenter] removeObserver:self];
}
+ (instancetype)showFromImageView:(UIImageView *)imageView withURLStrings:(NSArray *)URLStrings placeholderImage:(UIImage *)image atIndex:(NSInteger)index dismiss:(DismissBlock)block  iscloseBtn:(BOOL)isColseBtn{
    YHHUPhotoBrowser *browser = [[YHHUPhotoBrowser alloc] initWithFrame:kScreenRect];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YHHUPhotoBrowserCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kPhotoBrowserCellID forIndexPath:indexPath];
    ...
    if (self.URLStrings) {
        ...
        if (indexPath.row != _index) {
            ...
            [cell.imageView hu_setImageWithURL:url placeholderImage:((placeHolder)? (placeHolder) : (_placeholderImage)) completed:^(UIImage *image, NSError *error, NSURL *imageUrl) {
                if (!_imageDidLoaded) {
                    _imageDidLoaded = YES;
...
- (void)configureBrowser {
    ...
    [self.collectionView registerClass:[YHHUPhotoBrowserCell class] forCellWithReuseIdentifier:kPhotoBrowserCellID];
...
- (void)dismiss {
    ...
    if (self.dismissDlock) {
        YHHUPhotoBrowserCell *cell = (YHHUPhotoBrowserCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:_currentPage inSection:0]];
