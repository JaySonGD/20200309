...
- (NSString *)getLetter
{
    ...
    NSString *letters = @"";
    for (NSUInteger i = 0 ; i < words.length ; i++) {
        NSString *result = nil;
        unichar letter = [words characterAtIndex:i];
        ...
        if (index >= 0 && index <= HANZI_COUNT) {
            result = [NSString stringWithFormat:@"%c", firstLetterArray[index]];
                   ...
                   || (letter >= 'A' && letter <= 'Z')) {
            result = [NSString stringWithFormat:@"%c", letter];
        } else {
            result = @"#";
        }
        letters = [NSString stringWithFormat:@"%@%@", letters, [result uppercaseString]];
    }
    return letters;
}
...
- (NSString *)firstCharactor:(NSString *)string
{
    ...
    //转化为大写拼音
    NSString *pinYin = [preString capitalizedString];
    //获取并返回首字母
...
- (NSArray *)arrayWithPinYinFirstLetterFormat:(NSString* (^)(id info))sign
{
    ...
    for (NSString *words in self) {
        NSString *key = sign(words);
        if (![key isKindOfClass:[NSString class]]) {
            return [NSMutableArray array];
        }
        NSString *firstLetter = [key firstCharactor:key];
    ...
    for (int i = 'A'; i <= 'Z'; i++) {
        ...
        if ([array count]) {
            [array sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
                NSString *word1 = sign(obj1);
                NSString *word2 = sign(obj2);
