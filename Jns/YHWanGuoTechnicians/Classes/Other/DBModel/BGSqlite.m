...
+ (NSString *)BGSqliteCacheDirectory {
    ...
    return [NSString stringWithFormat:@"%@/Library/Caches/Sqlite/",NSHomeDirectory()];
}
...
+(BOOL)isExistSqliteWithClass:(Class)model_class{
    ...
    NSString* path = [NSString stringWithFormat:@"%@%@%@_v%@.sqlite",CacheDir,tableName,BGFile,@(version)];
...
+(NSString*)getSqlitePathWithClass:(Class)model_class{
    ...
    NSInteger version = [BGTool getIntegerWithKey:tableName];
    return  [NSString stringWithFormat:@"%@%@%@_v%@.sqlite",CacheDir,tableName,BGFile,@(version)];
