...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingImage:(UIImage *)image editingInfo:(nullable NSDictionary<NSString *,id> *)editingInfo {
    ...
    [YHPhotoManger saveImage:image subDirectory:self.billId fileName:imageName];
...
- (UITableView *)tableView
{
    ...
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, screenWidth, screenHeight - kStatusBarAndNavigationBarHeight - kTabbarHeight) style:UITableViewStylePlain];

        
        YHChooseView *headerView = [YHChooseView chooseView];
        _tableView.tableHeaderView = headerView;
        headerView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 1380);
