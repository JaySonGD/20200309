...
+ (NSURL *)getURL:(NSString *)uri{
    //if ([self isDownloaded:uri]) {
        NSString *url  = [NSString stringWithFormat:@"http://127.0.0.1:9479/%@.m3u8",[[FFTSDownloader sharedManager] md5:uri]];
...
- (void)convert {
    ...
    NSLog(@"%s--- http://127.0.0.1:9479/%@ ", __func__,m3u8File);
    return;
