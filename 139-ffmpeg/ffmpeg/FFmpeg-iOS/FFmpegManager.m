...
@property (nonatomic, assign) float kbs;
@property (nonatomic, copy) void (^processBlock)(float process,float kbs,long long size);
...
+ (void)setTotalSize:(long long)size{
    [FFmpegManager sharedManager].fileSize = size;
}

+ (void)setKbs:(float)kbs{
    [FFmpegManager sharedManager].kbs = kbs;
}
...
+ (void)setCurrentTime:(long long)time {
    ...
    if (mgr.processBlock && mgr.fileDuration) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            mgr.processBlock(process,mgr.kbs,mgr.fileSize);
        });
    }
}
